...
export function VideoSlider({ videos, topContentComponent }: { videos: Videos[], topContentComponent: React.ReactNode }) {
  ...
  const handleDrag = (e: MouseEvent | TouchEvent) => {
    ...
    const newHeight = Math.min(Math.max(startHeightRef.current + deltaHeight, 20), 90)
  ...
  return (
    <div className="min-h-screen">
      ...
      <div
        ref={drawerRef}
        className="relative z-20 bg-gray-900 rounded-t-3xl overflow-hidden transition-all duration-300 ease-in-out"
        style={{ height: `${drawerHeight + 1.6}vh` }}
      >
        <div
          className="h-10 w-full flex items-center justify-center cursor-grab active:cursor-grabbing"
        ...
        >
          <GripHorizontalIcon className="text-gray-500" />
        </div>
        <ScrollArea className="h-[calc(100%-2.5rem)] overflow-x-hidden">
          <div className="grid grid-cols-2 gap-1 p-1">
            {videos.length
              ? videos.map((video) => (
                <VideoThumbnail key={video.id} video={video} onSelect={() => setSelectedVideo(video)} />
              ))
...
function VideoThumbnail({ video, onSelect }: { video: Videos; onSelect: () => void }) {
