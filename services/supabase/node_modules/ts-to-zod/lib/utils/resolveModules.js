...
const moduleToPrefix = (declarationNames) => (context) => (sourceFile) => {
    const prefixInterfacesAndTypes = (moduleName) => (node) => {
        ...
        if (typescript_1.default.isTypeAliasDeclaration(node)) {
            return typescript_1.factory.updateTypeAliasDeclaration(node, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.typeParameters, typescript_1.default.isTypeLiteralNode(node.type)
        ...
        if (typescript_1.default.isInterfaceDeclaration(node)) {
            return typescript_1.factory.updateInterfaceDeclaration(node, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.typeParameters, typescript_1.default.visitNodes(node.heritageClauses, prefixInterfacesAndTypes(moduleName)), typescript_1.default.visitNodes(node.members, prefixInterfacesAndTypes(moduleName)));
        }
        if (typescript_1.default.isHeritageClause(node)) {
            return typescript_1.factory.updateHeritageClause(node, typescript_1.default.visitNodes(node.types, prefixInterfacesAndTypes(moduleName)));
        }
        ...
        if (typescript_1.default.isEnumDeclaration(node)) {
            return typescript_1.factory.updateEnumDeclaration(node, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.members);
        }
