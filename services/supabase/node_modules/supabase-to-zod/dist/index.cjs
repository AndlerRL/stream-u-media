...
var getNodeName = (n) => {
  ...
  n.forEachChild((n2) => {
    ...
    if (import_typescript.default.isStringLiteral(n2)) {
      name = n2.text;
    }
  });
  if (!name)
...
var compositeTypeFormatterSchema = import_zod.z.function().args(import_zod.z.string()).returns(import_zod.z.string());
            ...
            if (import_typescript2.default.isTypeLiteralNode(n3)) {
              n3.forEachChild((n4) => {
                if (import_typescript2.default.isPropertySignature(n4) && import_typescript2.default.isIdentifier(n4.name)) {
                              ...
                              if (import_typescript2.default.isTypeLiteralNode(n7)) {
                                n7.forEachChild((n8) => {
                                  if (import_typescript2.default.isPropertySignature(n8)) {
                                    ...
                                    if (operation) {
                                      n8.forEachChild((n9) => {
                                        if (import_typescript2.default.isTypeLiteralNode(n9) || // Handle `Relationships` operation which is an array
                                        import_typescript2.default.isTupleTypeNode(n9)) {
                                          typeStrings.push(
                        ...
                        n5.forEachChild((n6) => {
                          ...
                          if (import_typescript2.default.isPropertySignature(n6)) {
                            n6.forEachChild((n7) => {
                              ...
                              if (import_typescript2.default.isLiteralTypeNode(n7)) {
                                const formattedName = enumFormatter(enumName);
                                typeStrings.push(
                                  `export type ${formattedName} = ${n7.getText(
                                    sourceFile
                  ...
                  if ("CompositeTypes" === n4.name.text) {
                    n4.forEachChild((n5) => {
                      if (import_typescript2.default.isTypeLiteralNode(n5)) {
                        n5.forEachChild((n6) => {
                          const enumName = getNodeName(n6);
                          if (import_typescript2.default.isPropertySignature(n6)) {
                            n6.forEachChild((n7) => {
                              if (import_typescript2.default.isTypeLiteralNode(n7)) {
                                const formattedName = compositeTypeFormatter(enumName);
                                typeStrings.push(
                                  `export type ${formattedName} = ${n7.getText(
                                    sourceFile
                                  )}`
...
async function supabaseToZod(opts) {
  ...
  const { getZodSchemasFile, errors } = (0, import_ts_to_zod.generate)({
